...
function Category({ categoryName, categoryDescription, topics }) {

  const topicArray = topics.map((topic) => {
    return (
      <div className="flex flex-col sm:relative sm:before:absolute sm:before:top-2 sm:before:w-4 sm:before:h-4 sm:before:rounded-full sm:before:left-[-35px] sm:before:z-[1] before:bg-violet-400" bis_skin_checked="1">


        <Link to={`/codewiki/blog/${topic.tag}`}>
        ...
        <div className="grid gap-4 mx-4 sm:grid-cols-12" bis_skin_checked="1">
          <div className="col-span-12 sm:col-span-3">
            <div className="text-center sm:text-left mb-14 before:block before:w-24 before:h-3 before:mb-5 before:rounded-md before:mx-auto sm:before:mx-0 before:dark:bg-violet-400" bis_skin_checked="1">
              ...
              <span className="text-sm font-bold tracking-wider uppercase dark:text-gray-400"> {categoryDescription} </span>
            </div>
          </div>
